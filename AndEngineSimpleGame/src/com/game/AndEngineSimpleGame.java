...
import java.io.IOException;
 ...
 import org.anddev.andengine.entity.sprite.Sprite; import org.anddev.andengine.entity.text.ChangeableText;
...
private TextureRegion mWinTextureRegion;
 ...
 @SuppressWarnings("deprecation")
  ...
  // setting up the camera [AndEngine's camera , not the one you take   // pictures with]
 ...
 @Override  public void onLoadResources() {
  ...
  // loading the image inside the container
   mPlayerTextureRegion = BitmapTextureAtlasTextureRegionFactory
     .createFromAsset(this.mBitmapTextureAtlas, this, "Player.png", 0, 0);
   ...
   mWinTextureRegion = BitmapTextureAtlasTextureRegionFactory
     .createFromAsset(this.mBitmapTextureAtlas, this, "win.png", 0,       128);
   mFailTextureRegion = BitmapTextureAtlasTextureRegionFactory
     .createFromAsset(this.mBitmapTextureAtlas, this, "fail.png", 0,       256);  
  // preparing the font
...
// TODO Auto-generated catch block
 ...
 @Override  public Scene onLoadScene() {
  ...
  // creating a new scene for the pause menu
   ...
   final int x = (int) (mCamera.getWidth() / 2 - mPausedTextureRegion
     ...
     .getHeight() / 2);   final Sprite pausedSprite = new Sprite(x, y, mPausedTextureRegion);
  ...
  // set background color
   ...
   mMainScene
     .setBackground(new ColorBackground(0f, 1.0f, 0f));
  ...
  // settings score to the value of the max score to make sure it appears   // correctly on the screen
 ...
 // TimerHandler for collision detection and cleaning up
  IUpdateHandler detect = new IUpdateHandler() {   @Override   public void reset() {   }  
  @Override   public void onUpdate(float pSecondsElapsed) {  
   ...
   // iterating over the targets
    ...
    // if target passed the left edge of the screen, then remove it     // and call a fail
     ...
     // if the targets collides with a projectile, remove the      // projectile and set the hit flag to true
 ...
 @Override  public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent) {  
...
projectile = new Sprite(player.getX(), player.getY(),
 ...
 // adds a target at a random location and let it move along the x-axis
  public void addTarget() {   Random rand = new Random();  
  int x = (int) mCamera.getWidth() + mTargetTextureRegion.getWidth();
   ...
   int rangeY = maxY - minY;   int y = rand.nextInt(rangeY) + minY;  
  ...
  MoveXModifier mod = new MoveXModifier(actualDuration, target.getX(),
     -target.getWidth());   target.registerEntityModifier(mod.deepCopy());  
...
Toast.LENGTH_SHORT).show();
     ...
     // in case the user clicks the home button while the game on the     // resultScene
     resumeMusic();
     mEngine.stop();
    }
   } else {
 ...
 public void pauseGame() {
   if (runningFlag) {
    mMainScene.setChildScene(mPauseScene, false, true, true);    mEngine.stop();
   }
